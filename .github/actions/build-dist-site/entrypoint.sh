...
ssh-agent bash -c 'ssh-add proba.txt; ssh-keyscan -H miserend.hu >> ~/.ssh/known_hosts; ssh-keyscan -H miserend.hu >> ~/etc/ssh/known_hosts; ssh elek@miserend.hu -o StrictHostKeyChecking=no -v; git push --force ssh://elek@miserend.hu/home/elek/miserend.git master:master'
echo "Próba"
